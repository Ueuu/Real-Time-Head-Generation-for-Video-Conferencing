...
class PredictorLocal:
    ...
    def predict(self, driving_frame):
        ...
        #print("driving frame {} bytes".format(sys.getsizeof(driving_frame)))
        
        with torch.no_grad():
            driving = to_tensor(driving_frame).to(self.device)

            #msg = pickle.dumps(driving)
            #print(" driving {} bytes".format(sys.getsizeof(msg)))
            #print(type(msg))
            ...
            #print("out before numpy {} bytes".format(sys.getsizeof(out)))
            #info(type(out))
